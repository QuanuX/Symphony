...
The implemented user-scope `qxctl knowledge session ...` surface revalidates the same exact coordinator binding and complete engine inventory, obtains a fresh exact SSIAG decision for every operation, and invokes the coordinator with the common session command. `begin` requires `absent` or the exact digest of a closed predecessor; mutations require a stable operation ID and exact current digest; explicit discovery recovery is permitted only when one unique forward state is provable. `status` is read-only. This surface establishes and recovers noncanonical authority-epoch evidence only. It does not activate a receipt, invoke vector semantics, mutate canonical knowledge, administer policy/safeguards, use a credential provider, or dock with Maestro.
