...
The implemented `0.1.0-dev` foundation supports direct coordinator `inspect` and explicit-path read-only `check`, plus independently installed SKVI `inspect`, `check`, caller-declared `propose`, and disposable `project`, and SCLV `inspect`, `check`, provider-neutral `propose`, non-mutating `recover`, and disposable `project`. qxctl validates each exact receipt, owned paths, process identity, deadline, and response digest before presenting results. Success proves those bounded operations only; it does not prove authentication, an active session, lifecycle activation, docking, ratification, journal mutation, or apply authority.
