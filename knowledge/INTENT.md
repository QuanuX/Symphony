...
### Scope
Defines the overarching knowledge framework structure (`knowledge/`), its common companion surfaces, and autonomous vector surfaces including SKVI, SCLV, SODV, SACV, SSFV, SAV, SEV, SSIAG, and STAV.
...
### Relationship to SLANG and NAMESPACES
`knowledge/SLANG.md` is the SKV-owned platform-nomenclature companion: it routes a preferred term, concise orientation, example, and counterexample to the exact contract that owns the term's semantics. `knowledge/NAMESPACES.md` is the SKV-owned universal identity-family and namespace-delegation companion: it records family ownership while leaving grammar, allocation, lifecycle, and individual identities with the delegated contract. Neither file is a fifth member of the SKV Contract Quad, a new vector, a runtime service, or a substitute for vector truth.
...
`knowledge/ssfv/` owns canonical application-feature identity, semantics, hierarchy, lifecycle, distinctions, distributed record routing, coverage inventory, and graph-projection contracts. Its partial catalog contains ninety-one experimental records across the repository root and seventeen implemented owner scopes, with seventy-one ratified nested features and three proposal-only scopes explicitly excluded. The newest records capture the Accordare STAV runtime producer, exact grant administration, and manifest-declared knowledge-surface discovery while operational Keychain access remains disabled. Remaining nested feature, subfeature, and microfeature review is incomplete, so this incremental catalog is not a repository-completeness claim. SSFV references other vectors without duplicating their truth.
...
`knowledge/sev/` owns planned-change and encountered-novelty cases, impact assessment, disposition, dependency-ready-set, recalculation, verification, recovery-advice, and closure contracts. SCSEV is the first governed SEV profile and reuses SSFV feature administration, stable qxctl command truth, and engine-operation descriptors rather than creating a duplicate engine or registry. The documentation-only knowledge-surface evolution profile applies the existing SEV case operations to additions, changes, renames, supersessions, deprecations, retirements, and removals while preserving current-versus-history interpretation. It adds no operation or runtime authority. The first engine slice is report-only and proposal-only; durable external action remains separately authorized through qxctl and the knowledge coordinator.
