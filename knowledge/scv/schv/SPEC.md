...
Architect-ratified bounded source-knowledge development contract, implemented by the current source package `0.10.0-dev`. Hyperscaler and cloud-hosting family knowledge, preserving each provider vocabulary.
...
The exact domain key is `schv` and its parent domain is `scv`. Provider-native names, region identities, account identifiers and API versions remain qualified by their source. The domain definition selects knowledge scope, not permission or proof that an adapter understands every product. Shared code accepts explicit definitions; onboarding another provider does not require a vendor-specific switch in the common implementation.
