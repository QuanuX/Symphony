...
## Purpose
This platform contract is filed under SKV at `knowledge/platform/`; the repository root presents orientation and licensing.
...
First-party repository implementations, authoring tools, schemas, tests and acceptance helpers must not introduce Python source or an interpreter dependency. C++ owns the replacement authoring and process-test tools. This is a source-maintenance contract, not a restriction on user composition or explicitly selected user tools.
