...
The current partial semantic catalog contains ninety-one Architect-ratified experimental records across the repository root and seventeen implemented owner scopes. Seventy-one nested features are now recorded across seventeen owner scopes: qxctl, the knowledge-session coordinator, Maestro, the SSIAG foundation and macOS provider, the STAV append authority and Go protocol kernel, the Accordare STAV producer, the shared C++ foundation, the Symphony Validator, and the SKVI, SCLV, SACV, SAV, SODV, SEV, and SSFV engines. The newest records capture Accordare pre-mutation intent durability and native supervision alongside durable Named Version audit production and exact grant administration; earlier records retain SSIAG provider-binding, invariant, and validator-assurance truth. `COVERAGE.md` records that exact progress, the reviewed macOS adapter no-child disposition, explicit non-feature boundaries, the top-level source universe, three proposal-only exclusions, freshness rule, and the remaining conditions reserved for a future `complete` state.
