...
The current partial semantic catalog contains fifteen Architect-ratified experimental records across the repository root and fourteen implemented owner scopes. `COVERAGE.md` records the explicit top-level source universe, three proposal-only exclusions, freshness rule, and the stricter nested-review conditions reserved for a future `complete` state.
