...
The current partial semantic catalog contains 110 Architect-ratified experimental records across the repository root and 34 implemented owner scopes. Seventy-six nested features are now recorded across seventeen owner scopes: qxctl, the knowledge-session coordinator, Maestro, the SSIAG foundation and macOS provider, the STAV append authority and Go protocol kernel, the Accordare STAV producer, the shared C++ foundation, the Symphony Validator, and the SKVI, SCLV, SACV, SAV, SODV, SEV, and SSFV engines. The catalog includes eight SCV/family/provider engines, the optional SCV graph connector, all eight SHV owners and their qxctl administration, alongside the existing identity, audit and knowledge capabilities. `COVERAGE.md` records that exact progress, the reviewed macOS adapter no-child disposition, explicit non-feature boundaries, the top-level source universe, explicit proposal-only exclusions, freshness rule, and the remaining conditions reserved for a future `complete` state.
