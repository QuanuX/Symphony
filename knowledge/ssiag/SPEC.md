...
The request contains no subject or caller-class field. It binds a fresh UTC request and expiry, request/correlation IDs, operation, resource, audience, and scope. SSIAG derives the subject solely from the accepted connection's kernel-attested identity and evaluates only exact configured grants. Every allow or deny decision requires a committed safe STAV policy-decision event; an absent or failed append returns service unavailable and releases no decision.
