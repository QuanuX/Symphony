...
The forty exact common v1 schemas under `knowledge/schemas/v1/` govern process requests/responses, descriptors, receipts, bindings, reconciliation/session/lifecycle/Maestro operations, temporal encodings, and deterministic validation results/policies/baselines. Common v2 schemas provide immutable content-addressed package ownership plus side-by-side apply-journal/head truth. Report/apply journal persistence, bounded recovery, reviewed generic local actions, applied-state commitment, durable Maestro presence, exact validator packaging, and qxctl warning controls are implemented. Canonical knowledge apply and Maestro engine execution remain unavailable. Three SSIAG authorization schemas and each vector's own operation schemas remain owned by their applicable Contract Quad.
