...
- the common vector-engine process, descriptor, proposal, reconciliation/session/SSFV-maintenance journal, explicit session-transition result, provider-evidence, install-receipt, engine-binding-registry, desired-state/first-boot lifecycle, and docking/inventory identifier family;
...
SSIAG derives subjects from kernel peer credentials, evaluates explicit exact grants with a deny default, and returns short-lived non-transferable evidence only after a committed STAV append. The existing vector engines retain their bounded check/proposal/projection scopes, and SSFV remains an explicitly partial fifteen-record catalog with top-level owner-scope coverage and incomplete nested review. Generic staged receipt-v2 installation/uninstall, protected selection/activation, and Maestro docking-presence changes are implemented only within explicit lifecycle apply. Protected validator warning profiles and baselines are implemented separately and never change detection. Receipt-v1 mutation, downloads, arbitrary entry-point execution, live service activation, Maestro engine invocation/supervision, host boot-hook installation, coordinator-to-vector invocation, repository/system/TOPS engine-binding profiles, broader safeguard administration, programmatic canonical apply, external package-manager publication, unratified SSFV feature records, and SACV-governed HTTP API documents remain unimplemented or separately gated.

The forty-six exact common v1 schemas under `knowledge/schemas/v1/` govern process requests/responses, descriptors, receipts, bindings, reconciliation/session/SSFV-maintenance/lifecycle/Maestro operations, temporal encodings, and deterministic validation results/policies/baselines. Common v2 schemas provide immutable content-addressed package ownership plus side-by-side apply-journal/head truth. Report/apply/SSFV-maintenance journal persistence, bounded recovery, reviewed generic local actions, applied-state commitment, durable Maestro presence and derived inventory, exact validator packaging, and qxctl warning controls are implemented. Canonical knowledge apply and Maestro engine execution remain unavailable. Three SSIAG authorization schemas and each vector's own operation schemas remain owned by their applicable Contract Quad.
