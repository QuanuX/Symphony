...
p=argparse.ArgumentParser();p.add_argument('--engine',required=True);a=p.parse_args();calls=rejects=0
...
def call(op,p,fail=False):
 ...
 r=subprocess.run([a.engine],input=json.dumps(request),capture_output=True,text=True);calls+=1
