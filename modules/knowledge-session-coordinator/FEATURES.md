  ...
  "records": [
    {
      ...
      "evidence": [
        ...
        "tools/qxctl/cmd/qxctl/lifecycle_test.go verifies report-plan safety, dynamic scheduler invariants, strict lifecycle-journal IPC validation, disabled apply, critical-extension refusal, and lifecycle Cobra grammar.",
      ...
      "how": "The C++26 symphony-knowledge-session process statically links the shared foundation, accepts the common bounded process envelope, and keeps reconciliation contexts, authenticated authority epochs, and lifecycle boot streams separate. Each durable state stream uses a private no-follow lock, fsync-backed dual slots, an atomic head, content digests, stable operation IDs, exact compare-and-swap state, opaque-extension preservation, and evidence-based forward recovery. Its report-only lifecycle planner strictly validates caller-supplied desired and observed evidence, negotiates exact receipt/protocol capabilities, and derives stable forward/inverse actions from a dependency ready set. Its separately authorized lifecycle boot operation recomputes stable inventory, persists bounded noncanonical plan/checkpoint identity, retains one transaction across linked evidence-driven revisions, and repairs only a unique digest-linked forward state. Stable inventory excludes only collection-time document fields so timestamp refresh creates no journal churn. The planner isolates critical dependency cycles and localized blockers, reports noncritical dependency advisories, enforces declared component capabilities, binds exact receptor targets, and safely sequences package changes without performing discovery or action execution. Go qxctl revalidates its immutable coordinator binding, authenticates the TOPS-scoped SSIAG endpoint, obtains and validates exact audited decisions, manages protected per-TOPS desired profiles, scans only fixed-layout validated receipts under selected roots, invokes that exact coordinator, and strictly validates report and journal results. Its explicit transition adapter derives stable step identities from one host event ID and composes status, bounded recovery, close, begin, or checkpoint so interrupted login, refresh, and logout sequences resume from durable journal evidence.",
      ...
      "non_claims": [
        ...
        "Does not implement canonical apply; it mutates only protected noncanonical user-scope reconciliation, authenticated-session, and lifecycle-journal state.",
        ...
        "The coordinator does not collect lifecycle observations or administer desired profiles; qxctl performs those functions, while the coordinator persists only report-only plan identity and executes no lifecycle action.",
      ...
      "what": "Provides an independently installable administrative process that reports exact capabilities, computes deterministic snapshots, maintains one recoverable noncanonical reconciliation context per canonical worktree, maintains separately authorized noncanonical authority epochs per TOPS, subject, and repository, converts complete desired/observed lifecycle evidence into a deterministic non-mutating dependency plan, and durably journals report-only boot/replan/recovery state per TOPS and profile.",
      "when": "Runs only on explicit user-scope qxctl or exact process invocation under a bounded deadline. Reconciliation begins with an explicit inventory. Session and qxctl lifecycle operations each require fresh exact SSIAG evidence. An explicit host integration may submit a stable login, refresh, or logout event to qxctl; retry reuses that event ID and resumes from journal evidence. qxctl lifecycle report or boot re-reads its protected profile and re-observes configured roots on every call, then supplies complete digest-bound evidence to the planner. Boot durably advances only when stable evidence changes; status is read-only and recovery is explicit. No operation acquires lifecycle action authority.",
