...
int main(int argc, char** argv) {
    if (argc == 2) {
        ...
        if (argument == "--help") {
            std::cout << "Usage: symphony-knowledge-session [--help|--version|--descriptor]\n"
                         "Without arguments, reads one bounded process request from standard input.\n";
            return 0;
        }
        if (argument == "--version") {
            std::cout << session::engine_id << ' ' << session::engine_version << '\n';
            return 0;
        }
        if (argument == "--descriptor") {
            std::cout << session::descriptor().dump() << '\n';
            return 0;
        }
    ...
    try {
        ...
        const auto request = engine::parse_request(input, session::engine_id, engine::unix_time_ms());
        try {
            auto result = session::handle_request(request);
            std::cout << engine::serialize_response(engine::success_response(
                request, session::engine_id, session::engine_version, std::move(result)));
            return 0;
