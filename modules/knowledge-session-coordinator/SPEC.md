...
## Durable SAV Named Versions

`named_version_prepare`, `named_version_seal`, `named_version_alias`, `named_version_lookup`, `named_version_status`, and `named_version_recover` implement the noncanonical persistence half of the SAV Named Version lifecycle. qxctl resolves and revalidates exact SAV and coordinator bindings, obtains a fresh SSIAG decision for every operation, invokes SAV before prepare and after artifact-bearing reads, and carries the SAV receipt/executable evidence into the prepared proposal. The coordinator validates that evidence but does not decide composition truth.

Each TOPS maps opaquely beneath `<state-root>/symphony/knowledge-session-coordinator/accordare/v1/tops/<tops-key>/named-versions/`. A persistent no-follow lock serializes access. Immutable prepared proposals and content-addressed Named Version objects are selected by a synchronized dual-slot registry and atomic head. Every mutation requires `absent` or the exact current registry digest, binds a stable operation fingerprint, synchronizes bytes before selection, and preserves immutable objects across aliases, rollback selection, upgrade, uninstall, and recovery.

Seal accepts only the exact bytes and digest from a prepared SAV validation. IDs and digests are unique, every predecessor resolves to a sealed object, and every alias selects one exact matching ID/digest pair. An alias is mutable convenience state and never identity. Lookup can select by exact digest, exact ID, or alias; it cannot infer newest. Status is read-only. Recovery may adopt only one unique valid digest-linked forward registry and records its repair evidence. Unsafe ownership/modes, symlinks, stale expected state, incompatible capabilities or versions, unknown critical state, missing immutable objects, and ambiguous or divergent evidence fail closed without cleanup.

The registry and head are disposable selection evidence around immutable objects, not canonical SAV truth. `canonical_apply_enabled`, `canonical`, and `stav_append_enabled` remain false in every result. STAV event vocabulary and append closure are separately gated future work.
