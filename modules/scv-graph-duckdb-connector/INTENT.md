...
Supply an independently installable C++ connector that stores and indexes explicitly supplied SCV graph revisions in a caller-selected local DuckDB database and namespace. The connector preserves exact native graph identity and supports bounded retrieval, export, recoverable publication, inventory and caller-directed transfer planning. qxctl is the operating interface for human and agentic callers.
