...
int emit_error(const engine::Error& error) {
    try {
        std::cout << engine::serialize_response(engine::error_response(
            "unavailable", "unavailable", "unavailable", sev::engine_id,
            sev::engine_version, error.code(), error.what()));
...
int main(int argc, char** argv) {
    if (argc == 2) {
        ...
        if (argument == "--help") {
            std::cout << "Usage: symphony-sev [--help|--version|--descriptor]\n"
                         "Without arguments, reads one bounded process request from standard input.\n";
            return 0;
        }
        if (argument == "--version") { std::cout << sev::engine_id << ' ' << sev::engine_version << '\n'; return 0; }
        if (argument == "--descriptor") { std::cout << sev::descriptor().dump() << '\n'; return 0; }
    ...
    try {
        ...
        const auto request = engine::parse_request(input, sev::engine_id, engine::unix_time_ms());
        try {
            auto result = sev::handle_request(request);
            std::cout << engine::serialize_response(engine::success_response(
                request, sev::engine_id, sev::engine_version, std::move(result)));
            return 0;
        } catch (const engine::Error& error) {
            std::cout << engine::serialize_response(engine::error_response(
                request.request_id, request.correlation_id, request.operation,
                sev::engine_id, sev::engine_version, error.code(), error.what()));
            return error.exit_status();
        }
    } catch (const engine::Error& error) { return emit_error(error); }
    catch (const std::exception&) { return emit_error(engine::Error("internal.failure", "bounded request processing failed", 5)); }
}
