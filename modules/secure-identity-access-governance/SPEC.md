...
Safe metadata, audited authorization, protected local policy administration, receipt-v2 packaging, a module-owned foundational lifecycle adapter, and exact metadata-only provider trust implement the Architect-ratified architecture in `knowledge/ssiag/SPEC.md`. Kernel caller authentication, native supervision, exact deny-by-default evaluation, non-transferable capability evidence, local policy proposal/apply/recovery, mutually authenticated typed STAV submission, enrollment/supervisor observe-plan-apply-status-recover, and the receipt-bound mutually verified provider handshake are implemented. Local policy apply is operational and noncanonical; safeguard administration, canonical knowledge apply, credential delivery, operational provider operations, the synthetic secret channel, and the foundational lifecycle STAV reconciliation endpoint remain disabled.
...
`qxctl ssiag status|providers|doctor|policy ...|provider show|verify ... --tops-id UUID [--scope user|system]` resolves the same TOPS-isolated socket, rejects unsupported schemas, bounds files/responses, and binds every operation to a ready status response with the requested TOPS identity and scope. `provider show` binds the source-owned `engop:symphony:ssiag.provider.trust.show` operation and validates the safe trust snapshot. `provider verify` binds the Go `engop:symphony:ssiag.provider.trust.verify` operation and its nested Swift `engop:symphony:ssiag.provider.metadata-handshake`, sends the exact four-field verification request, and validates the exact read-only result. The generic provider list claims only configured metadata and never substitutes for adapter execution. Neither singular route mutates a binding, performs a Keychain operation, or enables operational access. `policy propose` consumes a bounded deny-by-default policy or reset intent; `apply` consumes the exact returned proposal; `recover` requires explicit evidence. qxctl accepts and prints no secret values.

## Provider Contract

Foundation provider entries remain descriptive until paired with one exact `provider-executable-trust.v1` declaration in the protected per-TOPS SSIAG configuration tree. That external declaration binds one adapter and one invoking foundation version; absence yields `unbound`. qxctl binding mutation, newest-version selection, and implicit fallback are not enabled.
