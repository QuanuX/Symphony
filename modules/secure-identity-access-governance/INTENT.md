...
Kernel peer identity, exact-grant deny-by-default authorization, non-transferable capability evidence, receipt-v2 package identity, transactional enrollment/supervisor lifecycle, supervision, safe STAV submission, protected local policy status/proposal/apply/recovery, and exact metadata-only provider mutual trust are implemented. The module-owned lifecycle adapter and provider process boundary are bounded stdin/stdout, exact-identity, fail-closed, and have no network listener. Ordinary lifecycle apply fails before mutation until a committed STAV receipt endpoint exists; explicit deferred audit is journaled and remains reconciliation-required. Purge is native-only in lifecycle v1. The policy overlay never rewrites enrolled config and never grants canonical knowledge apply. General safeguard administration, lifecycle audit reconciliation, credential/provider operations, secret delivery, and Keychain access remain disabled until their exact schemas, platform identities, lifecycle rules, and negative-test gates in `REQUIREMENTS.md` pass. Remote SSIAG, implicit fallback, network listeners, and any apply path that is not based on target-host permission remain unauthorized.
