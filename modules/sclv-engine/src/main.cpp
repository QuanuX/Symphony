...
int emit_error(const engine::Error& error) {
    try {
        std::cout << engine::serialize_response(engine::error_response(
            ...
            sclv::engine_id, sclv::engine_version, error.code(), error.what()));
...
int main(int argc, char** argv) {
    if (argc == 2) {
        ...
        if (argument == "--help") {
            std::cout << "Usage: symphony-sclv [--help|--version|--descriptor]\n"
                         "Without arguments, reads one bounded process request from standard input.\n";
            return 0;
        }
        if (argument == "--version") {
            std::cout << sclv::engine_id << ' ' << sclv::engine_version << '\n';
            return 0;
        }
        if (argument == "--descriptor") {
            std::cout << sclv::descriptor().dump() << '\n';
            return 0;
        }
    ...
    try {
        ...
        const auto request = engine::parse_request(input, sclv::engine_id, engine::unix_time_ms());
        try {
            auto result = sclv::handle_request(request);
            std::cout << engine::serialize_response(engine::success_response(
                request, sclv::engine_id, sclv::engine_version, std::move(result)));
            return 0;
