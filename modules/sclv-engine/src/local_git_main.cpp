...
namespace {
constexpr const char* adapter_id = "symphony-sclv-evidence-local-git";
...
int main(int argc, char** argv) {
    if (argc == 2) {
        ...
        if (argument == "--help") {
            std::cout << "Usage: symphony-sclv-evidence-local-git [--help|--version|--descriptor]\n"
    ...
    try {
        ...
        try {
            ...
            auto result = provider::normalize_local_git(request.payload, request.deadline_unix_ms);
