...
class InterfaceGeneration(unittest.TestCase):
    ...
    def test_historical_projection_is_frozen(self):
        ...
        self.assertEqual([len(gen.projection(self.manifest, f"0.{n}.0-dev")["operations"]) for n in range(1, 11)], [13, 17, 20, 21, 22, 26, 26, 28, 30, 30])
    ...
    def test_future_domain_does_not_change_old_admission_or_definition(self):
        ...
        next_release["version"] = "0.11.0-dev"
        self.manifest["releases"].append(next_release)
        self.manifest["current_release"] = "0.11.0-dev"
        self.manifest["domains"].append({"name": "scev-new-provider", "introduced_in": "0.11.0-dev"})
        ...
        self.assertIn("scev-new-provider", gen.projection(self.manifest, "0.11.0-dev")["domains"])
    ...
    def test_sixth_release_declaration_remains_exact(self):
        ...
        self.assertEqual(original["current_release"], "0.6.0-dev")
        self.assertEqual(gen.release_manifest(self.manifest, "0.6.0-dev"), original)
    ...
    def test_eighth_release_declaration_remains_exact(self):
        original = gen.read_json(ROOT / "modules/scv-engine/tests/fixtures/owner-interface-0.8.v1.json")
        self.assertEqual(original["current_release"], "0.8.0-dev")
        self.assertEqual(gen.release_manifest(self.manifest, "0.8.0-dev"), original)
        self.assertEqual(gen.projection(self.manifest, "0.8.0-dev"), gen.projection(original, "0.8.0-dev"))

    def test_ninth_release_declaration_remains_exact(self):
        original = gen.read_json(ROOT / "modules/scv-engine/tests/fixtures/owner-interface-0.9.v1.json")
        self.assertEqual(original["current_release"], "0.9.0-dev")
        self.assertEqual(gen.release_manifest(self.manifest, "0.9.0-dev"), original)
        self.assertEqual(gen.projection(self.manifest, "0.9.0-dev"), gen.projection(original, "0.9.0-dev"))
        self.assertNotIn("composition_bundle_workflow", gen.projection(self.manifest, "0.9.0-dev")["surfaces"])
        self.assertIn("composition_bundle_workflow", gen.projection(self.manifest, "0.10.0-dev")["surfaces"])
    ...
    def test_malformed_manifest_rejected(self):
        mutations = [
            ...
            lambda m: m.update(current_release="0.11.0-dev"),
