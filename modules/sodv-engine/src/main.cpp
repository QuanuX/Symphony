...
int emit_error(const engine::Error& error) {
    try {
        std::cout << engine::serialize_response(engine::error_response(
            "unavailable", "unavailable", "unavailable", sodv::engine_id,
            sodv::engine_version, error.code(), error.what()));
...
int main(int argc, char** argv) {
    if (argc == 2) {
        ...
        if (argument == "--help") {
            std::cout << "Usage: symphony-sodv [--help|--version|--descriptor]\n"
                         "Without arguments, reads one bounded process request from standard input.\n";
            return 0;
        }
        if (argument == "--version") {
            std::cout << sodv::engine_id << ' ' << sodv::engine_version << '\n';
            return 0;
        }
        if (argument == "--descriptor") {
            std::cout << sodv::descriptor().dump() << '\n';
            return 0;
        }
    ...
    try {
        ...
        const auto request = engine::parse_request(input, sodv::engine_id, engine::unix_time_ms());
        try {
            auto result = sodv::handle_request(request);
            std::cout << engine::serialize_response(engine::success_response(
                request, sodv::engine_id, sodv::engine_version, std::move(result)));
            return 0;
        } catch (const engine::Error& error) {
            std::cout << engine::serialize_response(engine::error_response(
                request.request_id, request.correlation_id, request.operation,
                sodv::engine_id, sodv::engine_version, error.code(), error.what()));
            return error.exit_status();
        }
    } catch (const engine::Error& error) {
