...
bool strict_utc(std::string_view value) {
    ...
    const auto date = std::chrono::year{number(0U, 4U)} /
                      std::chrono::month{static_cast<unsigned int>(number(5U, 2U))} /
                      std::chrono::day{static_cast<unsigned int>(number(8U, 2U))};
    return date.ok() && number(11U, 2U) <= 23 && number(14U, 2U) <= 59 &&
           number(17U, 2U) <= 59;
}
