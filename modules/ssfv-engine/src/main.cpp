...
int emit_error(const engine::Error& error) {
    try {
        std::cout << engine::serialize_response(engine::error_response(
            "unavailable", "unavailable", "unavailable", ssfv::engine_id,
            ssfv::engine_version, error.code(), error.what()));
...
int main(int argc, char** argv) {
    if (argc == 2) {
        ...
        if (argument == "--version") {
            std::cout << ssfv::engine_id << ' ' << ssfv::engine_version << '\n';
            return 0;
        }
    ...
    try {
        ...
        try {
            ...
            std::cout << engine::serialize_response(engine::success_response(
                request, ssfv::engine_id, ssfv::engine_version, std::move(result)));
            return 0;
        } catch (const engine::Error& error) {
            std::cout << engine::serialize_response(engine::error_response(
                request.request_id, request.correlation_id, request.operation,
                ssfv::engine_id, ssfv::engine_version, error.code(), error.what()));
            return error.exit_status();
        }
    } catch (const engine::Error& error) {
