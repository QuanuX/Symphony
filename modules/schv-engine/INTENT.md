...
Provide independently installable C++26 source-knowledge computation for `schv` in the freezing path. Hyperscaler and cloud-hosting family knowledge, preserving each provider vocabulary. The shared implementation preserves native source meaning, durable provenance supplied by its storage adapter and reproducible graph projections. It is useful without a parent engine, provider credentials, an AI service, a database server or a network listener.
