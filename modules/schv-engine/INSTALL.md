...
```bash
cmake -S modules/schv-engine -B build/schv-engine -DBUILD_TESTING=ON
cmake --build build/schv-engine
ctest --test-dir build/schv-engine --output-on-failure
cmake --install build/schv-engine --prefix /chosen/prefix
```

The exact executable is `libexec/symphony/schv-engine/0.6.0-dev/symphony-schv` beneath the selected prefix. Install receipts and contract documents use the common exact `share` layout. Installation is inactive `installed_undocked`; it neither selects a binding nor installs another domain. Use an explicit prefix, exact version and selected domain through the admitted qxctl command surface. Direct `--descriptor` returns the supported operations.
