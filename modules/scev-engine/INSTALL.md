...
```bash
cmake -S modules/scev-engine -B build/scev-engine -DBUILD_TESTING=ON
cmake --build build/scev-engine
ctest --test-dir build/scev-engine --output-on-failure
cmake --install build/scev-engine --prefix /chosen/prefix
```

The exact executable is `libexec/symphony/scev-engine/0.6.0-dev/symphony-scev` beneath the selected prefix. Install receipts and contract documents use the common exact `share` layout. Installation is inactive `installed_undocked`; it neither selects a binding nor installs another domain. Use an explicit prefix, exact version and selected domain through the admitted qxctl command surface. Direct `--descriptor` returns the supported operations.
