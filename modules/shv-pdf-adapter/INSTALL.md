...
`SYMPHONY_KVE_USE_INSTALLED=ON` selects an independently installed foundation through CMake package discovery. The default builds the repository foundation. The runtime layout is `libexec/symphony/shv-pdf-adapter/0.1.0-dev/symphony-shv-pdf`; receipts bind installed files. `uninstall-shv-pdf-adapter` removes only verified owned files.
