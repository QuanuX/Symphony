...
- [`knowledge/`](knowledge/) contains the canonical SKV surfaces currently established for source routing (SKVI), change truth (SCLV), API governance (SACV), publication governance (SODV), semantic feature truth (SSFV), SSIAG, STAV, and the cross-vector desired-state/first-boot lifecycle topology. Canonical knowledge governs implementations; tools do not own canonical schemas.
