...
- [SSFV Engine](modules/ssfv-engine/) is an independently installable C++26 semantic-feature engine implementing structural and freshness-aware checks, content-addressed diffs, caller-declared proposals, and disposable deterministic graphs. Its partial catalog records forty-three ratified experimental records across the root and fourteen implemented scopes, including twenty-eight exact nested F1 and F2 subfeatures, with explicit proposal-only and reviewed non-feature dispositions and remaining nested review still outstanding; the engine does not decide feature-worthiness or write canonical truth.
