...
- [SSFV Engine](modules/ssfv-engine/) is an independently installable C++26 semantic-feature engine implementing structural and freshness-aware checks, content-addressed diffs, caller-declared proposals, and disposable deterministic graphs. Its partial catalog records twenty-nine ratified experimental records across the root and fourteen implemented scopes, including fourteen exact nested F1 subfeatures, with explicit proposal-only exclusions and remaining nested review still outstanding; the engine does not decide feature-worthiness or write canonical truth.
