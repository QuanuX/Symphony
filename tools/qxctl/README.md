...
### Linux report-only boot receptor

`knowledge lifecycle host install|update|status|reconcile|enable|disable|uninstall|run` administers or dispatches one explicit system-scoped systemd receptor per TOPS/profile. Install and update require `--expected-host-digest`; the default system state base is `/var/lib`, and `--integration-root` may select a different protected executor root at installation. Relocation uses explicit uninstall followed by install so no old root becomes untracked. qxctl copies its administrator-owned protected executable into a content-addressed slot and retains up to eight explicitly ordered digest-valid fallbacks, so a module/qxctl update in either order remains fail-closed and recoverable without selecting a newest version.

The unit calls the internal `host run` operation once per Linux kernel boot UUID. It checks the protected descriptor and executing qxctl digest, obtains fresh exact SSIAG decisions, optionally performs only uniquely linked discovery recovery, reads applied-state evidence, and invokes report-only lifecycle boot. It never invokes `lifecycle apply`. `host reconcile` repairs unit or enablement drift, promotes an accepted fallback if the active slot is unavailable, and completes interrupted `retiring` cleanup. Native Windows and launchd receptors, login/session hooks, and hidden watchers are not implemented.
...
# Read one SSIAG-owned provider trust snapshot
go run ./cmd/qxctl ssiag provider show PROVIDER --tops-id UUID [--json] [--scope user|system]

# Request fresh SSIAG-owned provider trust verification
go run ./cmd/qxctl ssiag provider verify PROVIDER --tops-id UUID [--authority-basis host_owner|granted_permission] [--json] [--scope user|system]

# Inventory exact SSIAG-owned provider installations
go run ./cmd/qxctl ssiag provider installations PROVIDER --tops-id UUID [--json] [--scope user|system]

# Inspect, plan, apply, observe, or recover an exact provider binding
go run ./cmd/qxctl ssiag provider binding status PROVIDER --tops-id UUID [--json] [--scope user|system]
go run ./cmd/qxctl ssiag provider binding plan PROVIDER --tops-id UUID --installation-id sha256:... --expected-state-digest absent --reason "activate exact installation" --json
go run ./cmd/qxctl ssiag provider binding apply PROVIDER --tops-id UUID --plan-digest sha256:... --expected-state-digest absent --json
go run ./cmd/qxctl ssiag provider binding apply-status PROVIDER --tops-id UUID --operation-id UUID --json
go run ./cmd/qxctl ssiag provider binding recover PROVIDER --tops-id UUID --expected-state-digest sha256:... --reason "resume exact attempt" --json
...
# Inspect the canonical common invariant registry without claiming semantic validity
go run ./cmd/qxctl knowledge invariant status [--repo /repository] [--json]
go run ./cmd/qxctl knowledge invariant list [--repo /repository] [--json]
...
# Invoke exact independently installed SAV and SEV engines
go run ./cmd/qxctl sav current --prefix /chosen/prefix --host --tops-id UUID [--state-root PATH] [--maestro-prefix PATH] --json
...
go run ./cmd/qxctl validate warning sync --tops-id 018f0c3a-7b2d-7e11-8c12-0242ac120002 --prefix /chosen/prefix --warning-state-id default --expected-state-digest absent --repo /absolute/path/to/Symphony
go run ./cmd/qxctl validate warning accept --tops-id 018f0c3a-7b2d-7e11-8c12-0242ac120002 --warning-state-id default --subject-id sha256:... --expected-state-digest sha256:... --rationale "reviewed owner evidence" [--valid-until 2026-09-01T00:00:00Z]
go run ./cmd/qxctl validate debug --tops-id 018f0c3a-7b2d-7e11-8c12-0242ac120002 --prefix /chosen/prefix --warning-state-id default --classification resolved --subject sha256:...
go run ./cmd/qxctl validate root-summary --prefix /chosen/prefix --repo /absolute/path/to/Symphony [--json]
```

SSIAG commands require an immutable TOPS UUID through `--tops-id` or `SYMPHONY_SSIAG_TOPS_ID`. They use `SYMPHONY_SSIAG_SOCKET` only as an explicit override; otherwise the selected scope and TOPS ID determine the isolated socket. `providers` remains the configured metadata list and does not claim a live adapter exchange. Singular `provider show` reads a digest-bound SSIAG trust snapshot; `provider verify` requests a fresh, permission-backed trust evaluation through SSIAG and records both the Go trust-verification identity and its nested Swift metadata-handshake identity. `provider installations` lists SSIAG-issued safe metadata and opaque installation digests. The five `provider binding` leaves read state, create a non-mutating plan, apply it through SSIAG's STAV-before-commit compare-and-swap transaction, inspect one exact attempt, or resume its unique evidence-linked recovery. Multiple versions may coexist; installation order, timestamps, and semantic recency never select one. qxctl does not open provider receipts, accept adapter paths or raw versions, launch an adapter, edit STAV, contact a keyring, or transport provider payloads, operational enablement, or credential values. It validates exact response shape, TOPS/scope binding, whole-second UTC timestamps, lowercase digests, attempt stages, false operational flags, and omit-self evidence digests before presentation.
...
Validator warning sensitivity is administered without changing the detector: `record` retains a warning, `review` signals new matching warnings, and `require` fails on new matching warnings. Baselines retain the exact v1 occurrence delta. The side-by-side warning-state protocol groups occurrences by stable subject and classifies subjects as `open`, `accepted`, `resolved`, or `superseded`; acceptance requires rationale and may expire at a future STSC whole second. Presentation-only mute never changes actionability, raw evidence, policy outcome, or debug visibility. Ordinary prose shows actionable open warnings, while exact list/show/debug queries retain occurrence bodies, evidence digests, classifications, and linked transition history. Violations, parser bounds, path safety, atomic writes, expected-state validation, ledger framing, and secret exclusion are never optional. Broader safeguard management, canonical apply, and audit-deferred recovery remain unavailable.

Validation state is protected beneath `<state-root>/symphony/<tops-id>/qxctl/validation/`. Mutations require `absent` or the exact current digest and use caller-neutral owner-only, no-follow, synchronized atomic replacement. A baseline acknowledges observed evidence; it does not ratify, delete, or resolve a warning. Warning resolution is derived only from a later complete detector result. A repository-identity or validator-version mismatch fails closed. `validate root-summary` separately invokes the exact receipt-owned validator with an empty environment and deadline, validates the strict `symphony.repository.root-summary.v1` digest, and returns validator projection failure as exit 25 without inventing partial evidence or writing README.
...
## SCV provider authoring and composition (`0.6.0-dev`)

`scv provider pack prepare` seals a caller-authored provider pack and any explicitly supplied detached fixture inputs. `scv provider pack evaluate` interprets the selected captured bytes, compares selected fixtures with their authored expectations and returns replayable qualified knowledge. Fixture agreement is evidence about the selected cases. Provider IDs and source locators remain caller declarations within the invoked engine's published domain boundary.

`scv composition explore` evaluates the finite combinations of caller-supplied recipes against explicit requirements and selected evidence at one query time. Required and optional checks, prerequisites, declared interface compatibility, guarantee changes and authored resolution pointers remain visible. Explicit counterfactual requirements are evaluated separately. `scv composition reassess` replays exact before/after results and reports the changed inputs. Neither command chooses a user's preferred provider, invents a recipe or provisions a system.

The four operations accept bounded no-follow JSON through `--input FILE`; use `--prefix PATH`, `--domain DOMAIN` and exact `--version 0.6.0-dev` to select a separately installed owner. Normal SCV structured failures remain documented in `ERRORS.md`. The metadata reader `scv interface show --prefix PATH --domain DOMAIN --version 0.6.0-dev --json` verifies the receipt-owned owner declaration and reports both its retained file digest and canonical definition digest. It uses no checkout and does not execute the declaration. Receipt-owned schemas and authoring templates are available through `scv schema list|show|template` with the same explicit `.6` selection.

`scv artifact import` admits `provider_pack`, `provider_pack_evaluation`, `composition` and `composition_reassessment` records in `.6`. Supply the exact operation and input with `result:null` to invoke and retain the result, or supply an exact result for native replay comparison. `show` and later use validate the recorded original owner installation; metadata listing explicitly reports that native replay has not been performed. Retained evidence remains outside immutable package files. Existing source/corpus/workflow commands and earlier exact version defaults continue to follow their published scopes.

`knowledge/scv/OWNER-INTERFACE.json` generates operation/release admission and installation metadata; C++ handlers and independent Go semantic checks retain ownership of behavioral validation. Python is an optional source-authoring/check tool and is unnecessary for a normal build or installed invocation. `knowledge/scv/PROVIDER-PACKS.md`, `COMPOSITION.md` and `OWNER-INTERFACE.md` document the owner contract and limits. This remains a bounded development increment, not general Composer search or a deployment engine.
