...
SSIAG commands require an immutable TOPS UUID through `--tops-id` or `SYMPHONY_SSIAG_TOPS_ID`. They use `SYMPHONY_SSIAG_SOCKET` only as an explicit override; otherwise the selected scope and TOPS ID determine the isolated socket. `providers` remains the configured metadata list and does not claim a live adapter exchange. Singular `provider show` reads a digest-bound SSIAG trust snapshot; `provider verify` requests a fresh, permission-backed trust evaluation through SSIAG and records both the Go trust-verification identity and its nested Swift metadata-handshake identity. Their bounded online command budget reserves two seconds for endpoint-status validation, seven seconds for the provider request (the foundation's five-second exchange plus two seconds of response and transport margin), and ten seconds end to end. qxctl does not read provider receipts, select an adapter version, launch an adapter, or contact a keyring. Both singular commands reject noncanonical, non-lowercase SHA-256, or secret-bearing results, and no SSIAG command accepts or prints credential values.
