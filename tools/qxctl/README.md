...
`knowledge/scv/OWNER-INTERFACE.json` generates operation/release admission and installation metadata; C++ handlers and independent Go semantic checks retain ownership of behavioral validation. C++ authoring executables generate and check the source metadata; no Python implementation or dependency is used. `knowledge/scv/PROVIDER-PACKS.md`, `COMPOSITION.md` and `OWNER-INTERFACE.md` document the owner contract and limits. This remains a bounded development increment, not general Composer search or a deployment engine.
