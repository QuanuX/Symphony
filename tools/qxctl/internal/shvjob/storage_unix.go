...
// Narrowly attributed copy of internal/shvstate/storage_unix.go filesystem mechanics.
// Job semantics and namespace are separate; source activation files are unchanged.
...
func (s Store) withDirectory(operation func(func() ([]byte, error), func([]byte, func() error) error) error) error {
	...
	if !filepath.IsAbs(root) || root == "/" || root != s.Root {
		return fmt.Errorf("materialization state root must be a clean absolute descendant path")
	}
	...
	identity, _ := knowledgeengineDigestIdentity(s.JobID)
	for _, part := range []string{"symphony", "qxctl", "shv", "materializations-v1", identity} {
	...
	defer unix.Close(fd)
	lockfd, err := unix.Openat(fd, "job.lock", unix.O_CREAT|unix.O_RDWR|unix.O_CLOEXEC|unix.O_NOFOLLOW|unix.O_NONBLOCK, 0o600)
	if err != nil {
	...
	if err := unix.Flock(lockfd, unix.LOCK_EX|unix.LOCK_NB); err != nil {
		return fmt.Errorf("materialization store is busy or cannot be locked: %w", err)
	}
...
func knowledgeengineDigestIdentity(jobID string) (string, error) {
	// Hashing is solely an unambiguous filesystem key, not source identity.
	digest, err := seal(map[string]any{"job_id": jobID})
...
func openChild(parent int, name string, private bool) (int, error) {
	...
	if err != nil {
		return -1, fmt.Errorf("open materialization state directory %q: %w", name, err)
	}
	...
	if status.Uid != uint32(os.Geteuid()) && status.Uid != 0 || status.Mode&0o022 != 0 && !(status.Uid == 0 && status.Mode&unix.S_ISVTX != 0) {
		_ = unix.Close(fd)
		return -1, fmt.Errorf("untrusted materialization state ancestor")
	}
	if private {
...
func privateDirectory(fd int, strict bool) error {
	...
	if st.Mode&unix.S_IFMT != unix.S_IFDIR || st.Uid != uint32(os.Geteuid()) || st.Mode&0o022 != 0 || strict && st.Mode&0o077 != 0 {
		return fmt.Errorf("job directory must be owned and private")
	}
	return nil
}
func privateFile(fd int) error {
	...
	if st.Mode&unix.S_IFMT != unix.S_IFREG || st.Uid != uint32(os.Geteuid()) || st.Mode&0o077 != 0 || st.Nlink != 1 {
		return fmt.Errorf("materialization state file must be private, owned, regular and singly linked")
	}
	return nil
}
func readState(directory int) ([]byte, error) {
	...
	if len(data) > maxStoreBytes {
		return nil, fmt.Errorf("materialization store exceeds bound")
	}
	return data, nil
}
...
func writeStateGuarded(directory int, data []byte, beforePublication func() error) error {
	...
	if beforePublication != nil {
		if err := beforePublication(); err != nil {
			return fmt.Errorf("checkpoint publication installation guard failed; prior checkpoint is recoverable: %w", err)
		}
	}
	...
	if err := unix.Fsync(directory); err != nil {
		return fmt.Errorf("checkpoint commit durability uncertain; inspect/recover exact operation: %w", err)
	}
	return nil
}
