...
func intentFor(t *testing.T, id string, current json.RawMessage, publisher string) Intent {
	...
	hashPart := "sha256:" + strings.Repeat("c", 64)
	manifest := sealed(t, map[string]any{"protocol": "symphony.shv.partition-manifest.v1", "entries": []any{map[string]any{"partition_digest": hashPart, "partition": nil}}, "required_references": []any{}, "loaded_count": 0, "missing_count": 1, "complete_inventory": false, "reference_statuses": []any{}})
	policy := "allow"
	if publisher != "Vendor" {
		policy = "reject"
	}
	partPrefix := "/partition"
	partInst := knowledgeengine.Installation{Role: "shv-partition-engine", ModuleID: "shv-partition-engine", EngineID: "symphony-shv-partition", Version: "0.2.0-dev", Prefix: partPrefix, ReceiptPath: partPrefix + "/share/symphony/receipts/shv-partition-engine/0.2.0-dev/install-receipt.json", ReceiptDigest: hashPart, ReceiptProtocol: "symphony.knowledge.install-receipt.v2", ExecutablePath: partPrefix + "/libexec/symphony/shv-partition-engine/0.2.0-dev/symphony-shv-partition", ExecutableDigest: hashPart}
	def := map[string]any{"catalogue_id": "family", "tops_id": "00000000-0000-4000-8000-000000000001", "manifest": manifest, "policy": map[string]any{"missing_partitions": "allow", "missing_references": policy}, "partition_installation": object(t, raw(t, partInst)), "members": []any{}}
	source := sealed(t, map[string]any{"protocol": "symphony.shv.publication-head.v1", "definition": def, "generation": generation, "previous_digest": previous})
	kind := "publish"
	plan := sealed(t, map[string]any{"protocol": "symphony.shv.publication-plan.v1", "operation_id": id, "expected_state_digest": previous, "change_kind": kind, "reason": "Explicit candidate source change", "head": source})
	transition := sealed(t, map[string]any{"protocol": "symphony.shv.publication-transition.v1", "operation_id": id, "expected_state_digest": previous, "head": source})
	prefix := "/retained-historical-installation"
	hash := "sha256:" + strings.Repeat("a", 64)
	inst := knowledgeengine.Installation{Role: "shv-publication-engine", ModuleID: "shv-publication-engine", EngineID: "symphony-shv-publication", Version: "0.1.0-dev", Prefix: prefix, ReceiptPath: prefix + "/share/symphony/receipts/shv-publication-engine/0.1.0-dev/install-receipt.json", ReceiptProtocol: "symphony.knowledge.install-receipt.v2", ReceiptDigest: hash, ExecutablePath: prefix + "/libexec/symphony/shv-publication-engine/0.1.0-dev/symphony-shv-publication", ExecutableDigest: hash}
...
func decisionFixture(t *testing.T, s Store, a Attempt) json.RawMessage {
	...
	d := ssiagclient.AuthorizationDecision{Schema: "symphony.ssiag.authorization-decision.v1", DecisionID: "test-decision", RequestID: id, CorrelationID: a.CorrelationID, TOPSID: s.TOPSID, Subject: ssiagclient.DecisionSubject{ID: "test-owner", Kind: "host", Authority: "unix_peer_credentials"}, Target: ssiagclient.DecisionTarget{Operation: "symphony.shv.catalogue.publish", Resource: resource(s.TOPSID, s.CatalogueID), Audience: "qxctl", Scope: "tops:" + s.TOPSID}, Effect: "allow", ReasonCode: "symphony.ssiag.policy.exact-grant", AuthorityBasis: &basis, PolicyDigest: hash, ConfigDigest: hash, DecidedAt: now, ExpiresAt: &expires}
...
func TestSHVPublicationStateCASReplayAndHistory(t *testing.T) {
	s, i := fixture(t)
...
func TestSHVPublicationStateInterruptedPublicationAndExpiry(t *testing.T) {
	s, i := fixture(t)
	if e := s.WithLock(func(tx *Transaction) error {
		if _, e := tx.Prepare(i); e != nil {
			return e
...
func TestSHVPublicationStateForgedNonHeadAndPendingLineage(t *testing.T) {
	s, i := fixture(t)
	commitFixture(t, s, i)
	...
	pending := intentFor(t, "pending", head.Head, "Next")
	if e := s.WithLock(func(tx *Transaction) error {
	...
	source := object(t, head.Head)
	source["generation"] = 3
	source["previous_digest"] = object(t, head.Head)["digest"]
	definition := source["definition"].(map[string]any)
	definition["policy"].(map[string]any)["missing_references"] = "reject"
	src := sealed(t, source)
	p["head"] = src
	r["head"] = src
	...
	child := intentFor(t, "pending-child", pr.Head, "Third")
...
func TestSHVPublicationStateAuthorizationBinding(t *testing.T) {
	s, i := fixture(t)
	if e := s.WithLock(func(tx *Transaction) error {
		if _, e := tx.Prepare(i); e != nil {
			return e
...
func statePath(s Store) string {
	hash, _ := knowledgeengineDigestIdentity(s.TOPSID, s.CatalogueID)
	return filepath.Join(s.Root, "symphony/qxctl/shv/catalogues-v1", s.TOPSID, hash, "state.json")
}
func TestSHVPublicationStatePathAndHistoryBounds(t *testing.T) {
...
func TestSHVPublicationStateStrictDecodeAndLimits(t *testing.T) {
	s, i := fixture(t)
...
func TestSHVPublicationStateRootAndEmptySnapshot(t *testing.T) {
	s, _ := fixture(t)
	if e := s.WithLock(func(tx *Transaction) error {
	...
	for _, root := range []string{"/", s.Root + "/", s.Root + "/../elsewhere", "relative", s.Root + "\n"} {
		if _, e := New(root, s.TOPSID, s.CatalogueID); e == nil {
	...
	via, e := New(link, s.TOPSID, s.CatalogueID)
	if e != nil {
		t.Fatal(e)
	}
...
func TestSHVPublicationStateConcurrentExpectedHead(t *testing.T) {
	s, first := fixture(t)
	second := intentFor(t, "competing", nil, "Other")
	start := make(chan struct{})
	done := make(chan error, 2)
	for _, candidate := range []Intent{first, second} {
		go func(i Intent) {
			<-start
			done <- s.WithLock(func(tx *Transaction) error {
				if _, err := tx.Prepare(i); err != nil {
					return err
				}
				a, _ := tx.Attempt(i.OperationID)
				return tx.Commit(i.OperationID, decisionFixture(t, s, a), func() error { return nil })
	...
	if passed != 1 {
		t.Fatalf("competing commits succeeded: %d", passed)
	}
	if err := s.WithLock(func(tx *Transaction) error {
		if len(tx.History()) != 1 {
			t.Fatal("competing publication changed history twice")
		}
		return nil
	}); err != nil {
		t.Fatal(err)
