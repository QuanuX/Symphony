...
// Narrowly attributed copy of internal/scvstate/storage_unix.go filesystem mechanics.
// SHV semantic validation and namespace are separate; SCV files are unchanged.
...
func (s Store) withDirectory(operation func(func() ([]byte, error), func([]byte, func() error) error) error) error {
	...
	identity, _ := knowledgeengineDigestIdentity(s.TOPSID, s.CatalogueID)
	for _, part := range []string{"symphony", "qxctl", "shv", "catalogues-v1", s.TOPSID, identity} {
...
func knowledgeengineDigestIdentity(topsID, catalogueID string) (string, error) {
	// Hashing is solely an unambiguous filesystem key, not head identity.
	digest, err := seal(map[string]any{"tops_id": topsID, "owner_engine_id": "symphony-shv-publication", "catalogue_id": catalogueID})
...
func writeStateGuarded(directory int, data []byte, beforePublication func() error) error {
	...
	if beforePublication != nil {
		publicationBarrier("publication.before_guard")
	...
	if beforePublication != nil {
		publicationBarrier("publication.before_rename")
	}
	if err := unix.Renameat(directory, name, directory, "state.json"); err != nil {
		return err
	}
	if beforePublication != nil {
		publicationBarrier("publication.after_rename")
	}
