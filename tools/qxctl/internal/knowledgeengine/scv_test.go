...
func TestInstalledSCVRetainsExactOldAndNewOperationSets(t *testing.T) {
	...
	if fourth := os.Getenv("SYMPHONY_SCV_AGENT_PREFIX"); fourth != "" {
		installations = append(installations, struct {
			prefix, version string
			count           int
		}{fourth, "0.4.0-dev", 21})
	...
	if sixth := os.Getenv("SYMPHONY_SCV_COMPOSITION_PREFIX"); sixth != "" {
		installations = append(installations, struct {
			prefix, version string
			count           int
		}{sixth, "0.6.0-dev", 26})
	}
	if seventh := os.Getenv("SYMPHONY_SCV_COMPOSITION_WORKFLOW_PREFIX"); seventh != "" {
		installations = append(installations, struct {
			prefix, version string
			count           int
		}{seventh, "0.7.0-dev", 26})
	}

	if eighth := os.Getenv("SYMPHONY_SCV_OBLIGATION_PREFIX"); eighth != "" {
		installations = append(installations, struct {
			prefix, version string
			count           int
		}{eighth, "0.8.0-dev", 28})
	...
	if tenth := os.Getenv("SYMPHONY_SCV_BUNDLE_WORKFLOW_PREFIX"); tenth != "" {
		installations = append(installations, struct {
			prefix, version string
			count           int
		}{tenth, "0.10.0-dev", 30})
...
func TestSCVOperationVersionsAreExplicit(t *testing.T) {
	for _, version := range []string{"latest", "0.11.0-dev", "", "0.1.0"} {
