...
func shvPublicationDescriptor(p, r map[string]any, version string) error {
	...
	contracts := []any{"knowledge/shv/PUBLICATION.md@v1"}
	if !scvEqual(r["limits"], limits) || !scvEqual(r["contract_versions"], contracts) {
		return shvFail()
	}
	ops, ok := r["operations"].([]any)
	count := len(shvPublicationOutputs)
	if !ok || len(ops) != count {
		return shvFail()
	}
	seen := map[string]bool{}
	domain := "shv-publication"
	feature := "ssfv:symphony:shv-publication-engine"
	for _, v := range ops {
		o := shvMap(v)
		name := shvText(o["operation_name"])
		out, ok := SHVPublicationResultProtocol(name)
		interaction := "apply"
		...
		if name == "publication_plan" {
			interaction = "propose"
		}
		interactions := []any{interaction}
		if name == "publication_reduce" {
			interactions = []any{"apply", "recover"}
		}
		...
		if !ok || seen[name] || o["engine_operation_id"] != "engop:symphony:"+domain+"."+strings.ReplaceAll(name, "_", ".") || o["availability"] != "implemented" || !scvEqual(o["feature_ids"], []any{feature}) || o["input_protocol"] != SHVPublicationInputProtocol(name) || o["output_protocol"] != out || o["authorization_requirement"] != "none" || o["expected_state_required"] != false || o["thermal_path"] != "freezing" {
			return shvFail()
		}
		seen[name] = true
	}
