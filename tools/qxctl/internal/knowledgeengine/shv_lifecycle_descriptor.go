...
func shvLifecycleDescriptor(p, r map[string]any) error {
	...
	contracts := []any{"knowledge/shv/SOURCES.md@v1", "symphony.shv.source-revision.v1", "symphony.shv.source-capture.v1", "symphony.graph.exchange.v1"}
	if !scvEqual(r["limits"], limits) || !scvEqual(r["contract_versions"], contracts) {
		return shvFail()
	}
	ops, ok := r["operations"].([]any)
	count := len(shvSourceOutputs)
	if !ok || len(ops) != count {
		return shvFail()
	}
	seen := map[string]bool{}
	domain := "shv-source"
	feature := "ssfv:symphony:shv-source-engine"
	for _, v := range ops {
		o := shvMap(v)
		name := shvText(o["operation_name"])
		out, ok := SHVSourceResultProtocol(name)
		interaction := "invoke"
		if name == "inspect" || name == "source_status" {
			interaction = "inspect"
		}
		...
		if !ok || seen[name] || o["engine_operation_id"] != "engop:symphony:"+domain+"."+strings.ReplaceAll(name, "_", ".") || o["availability"] != "implemented" || !scvEqual(o["feature_ids"], []any{feature}) || o["input_protocol"] != SHVSourceInputProtocol(name) || o["output_protocol"] != out || o["authorization_requirement"] != "none" || o["expected_state_required"] != (name == "source_plan" || name == "source_reduce") || o["thermal_path"] != "freezing" {
			return shvFail()
		}
		seen[name] = true
	}
