...
func shvPDFDescriptor(p, r map[string]any, version string) error {
	...
	contracts := []any{"knowledge/shv/PDF-ADAPTER.md@v1", "symphony.shv.pdf-extraction.v1"}
	...
	domain := "shv-pdf"
	feature := "ssfv:symphony:shv-pdf-adapter"
	for _, v := range ops {
		o := shvMap(v)
		...
		if !ok || seen[name] || o["engine_operation_id"] != "engop:symphony:"+domain+"."+strings.ReplaceAll(name, "_", ".") || o["availability"] != "implemented" || !scvEqual(o["feature_ids"], []any{feature}) || o["input_protocol"] != SHVPDFInputProtocol(name) || o["output_protocol"] != out || o["authorization_requirement"] != "none" || o["expected_state_required"] != false || o["thermal_path"] != "freezing" {
			return shvFail()
		}
		seen[name] = true
	}
