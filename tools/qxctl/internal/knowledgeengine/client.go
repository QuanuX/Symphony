...
func Invoke(ctx context.Context, prefix, version, repositoryRoot, operation string, payload []byte) (Response, error) {
	if !safeVersion(version) {
		return Response{}, fmt.Errorf("invalid SKVI engine version")
	}
	if !safeToken(operation, 64) {
		return Response{}, fmt.Errorf("invalid SKVI operation")
	}
	if err := validateJSONObject(payload, maxRequestBytes); err != nil {
		return Response{}, fmt.Errorf("invalid SKVI operation payload: %w", err)
	}
	...
	if err != nil {
		return Response{}, fmt.Errorf("generate SKVI request identity: %w", err)
	}
	...
	if err != nil {
		return Response{}, fmt.Errorf("encode SKVI request: %w", err)
	}
	if len(encoded) > maxRequestBytes {
		return Response{}, fmt.Errorf("encoded SKVI request exceeds %d bytes", maxRequestBytes)
	}
	...
	if childContext.Err() != nil {
		return Response{}, fmt.Errorf("SKVI engine exceeded its hard process deadline: %w", childContext.Err())
	}
	if stdout.exceeded {
		return Response{}, fmt.Errorf("SKVI engine response exceeds %d bytes", maxResponseBytes)
	}
	if stderr.exceeded {
		return Response{}, fmt.Errorf("SKVI engine diagnostic output exceeds 65536 bytes")
	}

	response, responseErr := validateResponse(
		stdout.Bytes(), requestID, operation, version)
	if responseErr != nil {
		if runErr != nil {
			return Response{}, fmt.Errorf("SKVI engine process failed and returned an invalid response: %w", responseErr)
		}
		return Response{}, responseErr
	}
	if response.Outcome == "error" {
		if runErr == nil {
			return Response{}, fmt.Errorf("SKVI engine returned an error outcome with a successful process status")
		}
		var exitError *exec.ExitError
		if !errors.As(runErr, &exitError) || exitError.ExitCode() < 2 || exitError.ExitCode() > 5 {
			return Response{}, fmt.Errorf("SKVI engine returned an error outcome with an invalid process status")
		}
	...
	if runErr != nil {
		...
		if errors.As(runErr, &exitError) {
			return Response{}, fmt.Errorf("SKVI engine returned success with process status %d", exitError.ExitCode())
		}
		return Response{}, fmt.Errorf("execute SKVI engine: %w", runErr)
	}
	if len(stderr.Bytes()) != 0 {
		return Response{}, fmt.Errorf("SKVI engine emitted diagnostics during a successful operation")
	}
	return response, nil
}
...
func ReadPayload(path string) ([]byte, error) {
	if path == "" {
		return nil, fmt.Errorf("--input is required for SKVI propose")
	}
	abs, err := filepath.Abs(path)
	if err != nil {
		return nil, fmt.Errorf("resolve proposal input: %w", err)
	}
	requested, err := os.Lstat(abs)
	if err != nil || requested.Mode()&os.ModeSymlink != 0 || !requested.Mode().IsRegular() {
		return nil, fmt.Errorf("proposal input must be a no-follow regular file")
	}
	resolved, err := filepath.EvalSymlinks(abs)
	if err != nil {
		return nil, fmt.Errorf("canonicalize proposal input: %w", err)
	}
	...
	if err != nil || resolvedInfo.Mode()&os.ModeSymlink != 0 || !resolvedInfo.Mode().IsRegular() ||
		...
		return nil, fmt.Errorf("proposal input changed during canonicalization")
	}
	root, relative, err := splitAbsolutePath(resolved)
	if err != nil {
		return nil, fmt.Errorf("resolve proposal input root: %w", err)
	}
	data, err := readNoFollowRelative(root, relative, maxRequestBytes)
	if err != nil {
		return nil, fmt.Errorf("read no-follow proposal input: %w", err)
	}
...
func resolveInstalled(prefix, version string) (string, error) {
	if !safeVersion(version) {
		return "", fmt.Errorf("invalid SKVI engine version")
	}
	...
	receiptRelative := filepath.ToSlash(filepath.Join(
		"share", "symphony", "receipts", moduleID, version, "install-receipt.json"))
	binaryRelative := filepath.ToSlash(filepath.Join(
		"libexec", "symphony", moduleID, version, engineID))
	receiptBytes, err := readNoFollowRelative(prefix, receiptRelative, maxReceiptBytes)
	if err != nil {
		return "", fmt.Errorf("validate SKVI install receipt: %w", err)
	}
	if err := validateJSONObject(receiptBytes, maxReceiptBytes); err != nil {
		return "", fmt.Errorf("invalid SKVI install receipt JSON: %w", err)
	}
	if err := requireExactFields(receiptBytes, []string{
		...
		return "", fmt.Errorf("invalid SKVI install receipt fields: %w", err)
	}
	var installed receipt
	if err := decodeExact(receiptBytes, &installed); err != nil {
		return "", fmt.Errorf("decode SKVI install receipt: %w", err)
	}
	if installed.Protocol != receiptProtocol || installed.ModuleID != moduleID || installed.Version != version {
		return "", fmt.Errorf("SKVI install receipt identity mismatch")
	}
	if installed.InstallScope != "prefix" || installed.PrefixMode != "installation_prefix" ||
		...
		return "", fmt.Errorf("SKVI install receipt lifecycle state is not the inactive undocked contract")
	}
	expected := expectedFiles(version)
	if len(installed.Files) != len(expected) {
		return "", fmt.Errorf("SKVI install receipt file set has %d entries, want %d", len(installed.Files), len(expected))
	}
	seen := make(map[string]struct{}, len(installed.Files))
	for _, relative := range installed.Files {
		if !safeRelativePath(relative) {
			return "", fmt.Errorf("SKVI install receipt contains an unsafe path")
		}
		if _, duplicate := seen[relative]; duplicate {
			return "", fmt.Errorf("SKVI install receipt contains a duplicate path")
		}
		seen[relative] = struct{}{}
		if _, ok := expected[relative]; !ok {
			return "", fmt.Errorf("SKVI install receipt contains an unexpected path: %s", relative)
		}
	...
	for relative := range expected {
		if _, ok := seen[relative]; !ok {
			return "", fmt.Errorf("SKVI install receipt is missing required path: %s", relative)
		}
	}
	...
	if err != nil || info.Mode()&os.ModeSymlink != 0 || !info.Mode().IsRegular() || info.Mode().Perm()&0o111 == 0 {
		return "", fmt.Errorf("SKVI installed engine is not a no-follow executable regular file")
	}
	return binary, nil
}
...
func validateResponse(data []byte, requestID, operation, version string) (Response, error) {
	if err := validateJSONObject(data, maxResponseBytes); err != nil {
		return Response{}, fmt.Errorf("invalid SKVI engine response: %w", err)
	}
	if err := requireExactFields(data, []string{
		...
		return Response{}, fmt.Errorf("invalid SKVI engine response fields: %w", err)
	}
	var response Response
	if err := decodeExact(data, &response); err != nil {
		return Response{}, fmt.Errorf("decode SKVI engine response: %w", err)
	}
	if response.Protocol != processProtocol || response.RequestID != requestID ||
		...
		response.EngineID != engineID || response.EngineVersion != version {
		return Response{}, fmt.Errorf("SKVI engine response identity mismatch")
	}
	if !taggedDigest(response.ResponseDigest) {
		return Response{}, fmt.Errorf("SKVI engine response digest has invalid syntax")
	}
	var object map[string]any
	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.UseNumber()
	if err := decoder.Decode(&object); err != nil {
		return Response{}, fmt.Errorf("decode SKVI response digest input: %w", err)
	}
	delete(object, "response_digest")
	canonical, err := marshalCanonical(object)
	if err != nil {
		return Response{}, fmt.Errorf("canonicalize SKVI response: %w", err)
	}
	digest := sha256.Sum256(canonical)
	if response.ResponseDigest != "sha256:"+hex.EncodeToString(digest[:]) {
		return Response{}, fmt.Errorf("SKVI engine response digest mismatch")
	}
	switch response.Outcome {
	case "ok":
		if response.Error != nil || bytes.Equal(response.Result, []byte("null")) || len(response.Result) == 0 {
			return Response{}, fmt.Errorf("SKVI success response has invalid result/error state")
		}
		if err := validateJSONObject(response.Result, maxResponseBytes); err != nil {
			return Response{}, fmt.Errorf("SKVI success result is invalid: %w", err)
		}
	case "error":
		if response.Error == nil || !bytes.Equal(response.Result, []byte("null")) ||
			...
			return Response{}, fmt.Errorf("SKVI error response has invalid result/error state")
		}
	default:
		return Response{}, fmt.Errorf("SKVI engine response has unknown outcome")
	}
	return response, nil
}
