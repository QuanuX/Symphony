...
func validateIntentShape(i Intent) error {
	...
	x := i.Installation
	expectedReceipt := filepath.Join(x.Prefix, "share/symphony/receipts/shv-source-engine", x.Version, "install-receipt.json")
	expectedExecutable := filepath.Join(x.Prefix, "libexec/symphony/shv-source-engine", x.Version, "symphony-shv-source")
	if x.Role != "shv-source-engine" || x.ModuleID != "shv-source-engine" || x.EngineID != "symphony-shv-source" || (x.Version != knowledgeengine.SHVSourceVersion && x.Version != knowledgeengine.SHVSourceInterfaceVersion) || !cleanRoot(x.Prefix) || x.ReceiptPath != expectedReceipt || x.ExecutablePath != expectedExecutable || x.ReceiptProtocol != "symphony.knowledge.install-receipt.v2" || !hashPattern.MatchString(x.ReceiptDigest) || !hashPattern.MatchString(x.ExecutableDigest) {
		return fmt.Errorf("SHV intent lacks exact historical owner installation")
	}
	var p retainedPlan
	var r retainedTransition
	if decode(i.Plan, &p) != nil || decode(i.Transition, &r) != nil || p.Protocol != "symphony.shv.source-plan.v1" || r.Protocol != "symphony.shv.source-transition.v1" || p.OperationID != i.OperationID || r.OperationID != i.OperationID || !sameDigest(p.Expected, r.Expected) || !sameJSON(p.Source, r.Source) {
...
func validateDocument(d Document, s Store) error {
	if d.Protocol != protocol || d.TOPSID != s.TOPSID || d.SourceID != s.SourceID || d.Operations == nil || len(d.Operations) > maxOperations {
	...
	for id, a := range d.Operations {
		...
		if json.Unmarshal(r.Source, &source) != nil || source.Definition.SourceID != s.SourceID || !hashPattern.MatchString(source.Digest) || !sameDigest(source.Previous, r.Expected) {
			return fmt.Errorf("retained SHV source identity/predecessor mismatch")
		}
		if a.Status == "committed" {
			if _, exists := committed[source.Digest]; exists {
				return fmt.Errorf("duplicate committed SHV source")
			}
			if _, exists := byGeneration[source.Generation]; exists {
				return fmt.Errorf("committed SHV chain forks")
			}
			committed[source.Digest] = r.Source
			byGeneration[source.Generation] = source.Digest
		}
	}
	...
	for _, a := range d.Operations {
		...
		if e = knowledgeengine.ValidateSHVSourceResultVersion("source_reduce", input, a.Intent.Transition, a.Intent.Installation.Version); e != nil {
			return fmt.Errorf("retained SHV transition replay: %w", e)
		}
	}
	if len(committed) == 0 {
		if d.StateDigest != nil || d.HeadOperationID != nil || !bytes.Equal(d.Source, []byte("null")) {
	...
	for generation := int64(1); generation <= int64(len(committed)); generation++ {
		...
		var source retainedSource
		_ = json.Unmarshal(committed[digest], &source)
		if generation == 1 {
			if source.Previous != nil {
				return fmt.Errorf("SHV initial predecessor mismatch")
			}
		} else if source.Previous == nil || *source.Previous != byGeneration[generation-1] {
			return fmt.Errorf("SHV committed history forks")
		}
	}
	if *d.StateDigest != byGeneration[int64(len(committed))] || !sameJSON(d.Source, committed[*d.StateDigest]) {
		return fmt.Errorf("SHV selected source is not terminal committed revision")
	}
	...
	if !ok || head.Status != "committed" || !sameJSON(transition.Source, d.Source) {
...
func documentHistory(d Document) ([]json.RawMessage, error) {
	...
	for _, a := range d.Operations {
		...
		if e := json.Unmarshal(r.Source, &s); e != nil {
			return nil, e
		}
		entries = append(entries, entry{s.Generation, append(json.RawMessage(nil), r.Source...)})
	}
...
func resource(tops, source string) string {
	d, _ := knowledgeengine.SCVDigest(map[string]any{"tops_id": tops, "owner_engine_id": "symphony-shv-source", "source_id": source})
...
func validateHistoryRaw(history []json.RawMessage) error {
	...
	if len(input) > maxHistoryPayloadBytes {
		return fmt.Errorf("SHV committed history exceeds native source_status request budget")
	}
	digests := []string{}
	...
	result := map[string]any{"protocol": "symphony.shv.source-status.v1", "source": history[len(history)-1], "history_digests": digests}
	...
	return knowledgeengine.ValidateSHVSourceResult("source_status", input, output)
}
