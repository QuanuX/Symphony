// Package shvstate is qxctl's protected filesystem transaction adapter. It does
...
const protocol = "symphony.qxctl.shv-source-store.v1"
...
type Store struct{ Root, TOPSID, SourceID string }
...
func New(root, topsID, sourceID string) (Store, error) {
	if !topsPattern.MatchString(topsID) || !tokenPattern.MatchString(sourceID) || !cleanRoot(root) || root == "/" {
		return Store{}, fmt.Errorf("invalid protected SHV root/TOPS/source identity")
	}
	return Store{Root: root, TOPSID: topsID, SourceID: sourceID}, nil
}
...
func (s Store) WithLock(operation func(*Transaction) error) error {
	if _, err := New(s.Root, s.TOPSID, s.SourceID); err != nil {
		return err
	}
	return s.withDirectory(func(read func() ([]byte, error), write func([]byte, func() error) error) error {
		...
		document := Document{Protocol: protocol, TOPSID: s.TOPSID, SourceID: s.SourceID,
...
func (t *Transaction) Prepare(intent Intent) (bool, error) {
	...
	prospective = append(prospective, proposed.Source)
...
func (t *Transaction) Commit(id string, authorization json.RawMessage, stillValid func() error) error {
	...
	next = cloneDocument(t.document)
	next.Operations[id] = attempt
	next.Source = append(json.RawMessage(nil), transition.Source...)
	var source retainedSource
	if err := json.Unmarshal(transition.Source, &source); err != nil {
		return err
	}
	next.StateDigest = &source.Digest
