...
func (s Store) withDirectory(operation func(func() ([]byte, error), func([]byte, func() error) error) error) error {
	...
	identity, _ := knowledgeengineDigestIdentity(s.TOPSID, s.SourceID)
	for _, part := range []string{"symphony", "qxctl", "shv", "sources-v1", s.TOPSID, identity} {
...
func knowledgeengineDigestIdentity(topsID, sourceID string) (string, error) {
	// Hashing is solely an unambiguous filesystem key, not source identity.
	digest, err := seal(map[string]any{"tops_id": topsID, "owner_engine_id": "symphony-shv-source", "source_id": sourceID})
