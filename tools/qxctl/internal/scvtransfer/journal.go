// Package scvtransfer owns local orchestration evidence. C++ connectors remain
...
const MaxBytes = 4 << 20
const Protocol = "symphony.qxctl.scv-index-transfer-intent.v1"
...
func ReadIntent(raw []byte) (Intent, error) {
	...
	if err = knowledgeengine.ValidateSCVGraphIndexResult("transfer_plan", in, i.Plan); err != nil {
		return i, err
	}
...
func ValidateChain(i Intent, events []Event) error {
	...
	for n, e := range events {
		...
		if e.Protocol != "symphony.qxctl.scv-index-transfer-event.v1" || e.TransferDigest != i.Digest || e.Sequence != n || e.Previous != prev || e.Phase != phase || e.OperationID != id || phase == "" {
