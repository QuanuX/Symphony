...
const Directory = "scv-transfer-journal"
...
func (j *Journal) Append(phase, id, result string) error {
	...
	e := Event{Protocol: "symphony.qxctl.scv-index-transfer-event.v1", TransferDigest: j.Intent.Digest, Sequence: len(j.Events), Previous: prev, Phase: phase, OperationID: id, ResultDigest: result}
	raw, err := Seal(e)
	if err != nil {
		return err
	}
