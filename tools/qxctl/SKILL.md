...
- SSIAG commands may read safe metadata, read exact provider trust snapshots, request fresh SSIAG-owned provider trust verification, and request exact audited authorization decisions. Keep `providers` as the metadata list; use singular `provider show|verify` only through SSIAG's authenticated endpoint. Never let qxctl inspect provider receipts, choose an adapter path/version/digest, launch an adapter, or contact a keyring. Treat every capability and trust result as non-transferable evidence for its bound operation, never a bearer token or canonical apply authority. Never pass secret values through qxctl arguments, input, output, logs, or fixtures.
