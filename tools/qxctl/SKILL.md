...
- `go run ./cmd/qxctl knowledge engines migrate --expected-registry-digest sha256:...`
...
- `go run ./cmd/qxctl sav named-version propose --tops-id UUID --input named-version.json --operation-id ID --expected-registry-digest absent --json`
...
- `go run ./cmd/qxctl sav named-version lookup --tops-id UUID --digest sha256:... --json`
...
- SSIAG commands may read safe metadata, read exact provider trust snapshots, request fresh SSIAG-owned provider trust verification, request three-layer signed-bundle/policy/session readiness evidence, inventory opaque exact provider installations, administer the closed provider-binding plan/apply/status/recovery circuit, and request exact audited authorization decisions. Keep `providers` as the metadata list; use singular `provider show|verify|readiness|installations` and `provider binding ...` only through SSIAG's authenticated endpoint. Readiness accepts no signer, requirement, path, profile, entitlement, provider payload, or operational-enable input and can never make an operation available. Binding plan accepts only an opaque installation digest or explicit `not_applicable`, exact prior state, and a bounded nonsecret reason; apply accepts only the SSIAG-issued plan digest and exact prior state. Never let qxctl open or interpret provider receipts, accept an adapter path or raw version, choose newest, launch an adapter, access a keyring, edit STAV, or transport operational enablement, provider payloads, or secrets. Treat every capability, trust, inventory, plan, readiness, and result object as non-transferable evidence for its bound operation, never a bearer token or independent canonical apply authority. Never pass secret values through qxctl arguments, input, output, logs, or fixtures.
- Canonical knowledge and STAV ledger surfaces remain read-only. Current mutations are limited to protected noncanonical state, exact local receipt-v2 packages, verified applied-state evidence, and the sole exact stopped-authority Accordare grant configuration route. Use only operations permitted by the target host and satisfy its configured safeguards; never emulate, manufacture, or bypass host authority.
- Core STAV read commands require an enrolled, running authority and an explicit reader grant. Accordare status/reconcile requires the exact enrolled producer and authenticated socket; reconcile replays only previously durable stable candidates. Accordare grant install/remove requires fresh SSIAG authorization, exact expected configuration state, a stopped authority, and the closed four-permission grant. Never bypass endpoint authentication, reader classification, or add arbitrary/raw append behavior.
...
- Use `knowledge engines bind` only with an exact inactive-undocked installation. Supply `absent` for the first v2 registry or the exact digest returned by `list` for every later mutation. A valid v1 registry remains readable but must be moved to a new linked v2 generation with explicit `knowledge engines migrate --expected-registry-digest DIGEST` before mutation. Migration copies exact binding values and never inspects installations or infers a version. A binding is noncanonical selection for later reconciliation; it is not installation, invocation, authentication, permission, repository activation, or Maestro docking.
...
- For SAV Named Versions, bind exact SAV and coordinator installations, reuse stable operation IDs only for identical retries, and supply `absent` or the exact registry digest for mutation. Treat prepare as immutable proposal persistence and seal as protected noncanonical object selection, not canonical apply. Revalidate every returned artifact through SAV; never infer newest, rewrite lineage, or treat an alias as identity. The coordinator-owned `stav_append_enabled=false` remains exact; when the producer is configured, qxctl reports the separate committed or durable-pending producer disposition instead of rewriting coordinator truth.
...
- For repository validation, use `validate scan` with one exact installed validator. Use `profile` to adjust optional warning disposition/presentation, `baseline` for exact occurrence deltas, and `warning sync|list|show|accept|reopen|supersede|mute|unmute` for subject-aware actionable lifecycle state. Acceptance requires rationale; mute is presentation-only; only complete detector evidence resolves or recurs a subject. `debug` filters display only after a complete scan. Use `validate root-summary` to inspect the strict receipt-backed repository summary; exit 25 is a projection failure, never permission to infer missing values or edit README. Never treat state as ratification, downgrade a violation, or alter detector scope.
...
- Do not use qxctl to replace `hotpath-runtime` or to revive the retired `node-troll` and `bus-troll` module identities.
