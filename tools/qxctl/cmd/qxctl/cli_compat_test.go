...
func TestCLICompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing SAV subcommand", args: []string{"sav"}, status: 1, output: "sav failed: SAV subcommand is required: inspect, reference-check, current, evaluate, diff, explain, graph, or compatibility\n"},
		{name: "SAV prefix required", args: []string{"sav", "inspect"}, status: 1, output: "sav inspect failed: --prefix is required\n"},
		{name: "missing SEV subcommand", args: []string{"sev"}, status: 1, output: "sev failed: SEV subcommand is required: inspect, case-open, impact, plan, verify, recalculate, status, recover, close, command-surface, graph, or compatibility\n"},
