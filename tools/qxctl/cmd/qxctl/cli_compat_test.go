...
func TestCLICompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing knowledge lifecycle subcommand", args: []string{"knowledge", "lifecycle"}, status: 1, output: "knowledge failed: knowledge lifecycle subcommand is required: profile, observe, report, boot, status, or recover\n"},
