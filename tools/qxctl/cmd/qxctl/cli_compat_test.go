...
func TestCLICompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing SEV subcommand", args: []string{"sev"}, status: 1, output: "sev failed: SEV subcommand is required: inspect, case-open, impact, plan, verify, recalculate, status, recover, close, command-surface, novelty-check, watch-check, trigger-coalesce, graph, or compatibility\n"},
