...
func TestCLICompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing knowledge session subcommand", args: []string{"knowledge", "session"}, status: 1, output: "knowledge failed: knowledge session subcommand is required: begin, status, checkpoint, close, or recover\n"},
