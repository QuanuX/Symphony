...
func TestCLICompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing SSIAG subcommand", args: []string{"ssiag"}, status: 1, output: "ssiag failed: SSIAG subcommand is required: status, providers, doctor, grants, or policy\n"},
		...
		{name: "missing knowledge lifecycle subcommand", args: []string{"knowledge", "lifecycle"}, status: 1, output: "knowledge failed: knowledge lifecycle subcommand is required: profile, ownership, observe, report, boot, status, recover, apply, apply-status, or apply-recover\n"},
		...
		{name: "missing validate subcommand", args: []string{"validate"}, status: 1, output: "validate failed: validate subcommand is required: scan, debug, profile, or baseline\n"},
