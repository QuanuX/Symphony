...
func TestCLICompatibility(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "missing knowledge engines subcommand", args: []string{"knowledge", "engines"}, status: 1, output: "knowledge failed: knowledge engines subcommand is required: list, inspect, doctor, bind, unbind, or migrate\n"},
		{name: "knowledge bind role required", args: []string{"knowledge", "engines", "bind"}, status: 1, output: help},
		{name: "knowledge migrate digest required", args: []string{"knowledge", "engines", "migrate"}, status: 1, output: "knowledge failed: --expected-registry-digest is required\n"},
		...
		{name: "missing SAV subcommand", args: []string{"sav"}, status: 1, output: "sav failed: SAV subcommand is required: inspect, reference-check, current, evaluate, diff, explain, graph, version-validate, version-diff, capsule-check, blueprint-plan, compatibility, or named-version\n"},
