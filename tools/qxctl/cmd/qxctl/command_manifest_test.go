...
func TestCommandRegistryCobraParityAndStableIdentity(t *testing.T) {
	...
	if len(manifest.Commands) != 369 {
		t.Fatalf("registered command count = %d, want 369", len(manifest.Commands))
	}
	...
	for _, required := range []string{
		...
		"qxcmd:symphony:shv.mapping.diagnose-source",
		"qxcmd:symphony:shv.references.analyze",
...
func TestCommandRegistryBindsVectorCapabilitiesNotBindingSelection(t *testing.T) {
	...
	want := map[string]string{
		...
		"qxcmd:symphony:sav.reference-check":       featureSAVCurrent,
		"qxcmd:symphony:sav.current":               featureSAVCurrent,
		"qxcmd:symphony:sav.evaluate":              featureSAVCurrent,
		"qxcmd:symphony:sav.diff":                  featureSAVCurrent,
		"qxcmd:symphony:sav.explain":               featureSAVCurrent,
		"qxcmd:symphony:sav.graph":                 featureSAVCurrent,
		"qxcmd:symphony:sav.version-validate":      featureSAVNamedVersion,
		"qxcmd:symphony:sav.version-diff":          featureSAVNamedVersion,
		"qxcmd:symphony:sav.capsule-check":         featureSAVCapsule,
		"qxcmd:symphony:sav.blueprint-plan":        featureSAVBlueprint,
		"qxcmd:symphony:sav.compatibility":         featureSAV,
		"qxcmd:symphony:sev.inspect":               featureSEV,
		"qxcmd:symphony:sev.case-open":             featureSEVEvolution,
		"qxcmd:symphony:sev.impact":                featureSEVEvolution,
		"qxcmd:symphony:sev.plan":                  featureSEVEvolution,
		"qxcmd:symphony:sev.verify":                featureSEVEvolution,
		"qxcmd:symphony:sev.recalculate":           featureSEVEvolution,
		"qxcmd:symphony:sev.status":                featureSEVEvolution,
		"qxcmd:symphony:sev.recover":               featureSEVEvolution,
		"qxcmd:symphony:sev.close":                 featureSEVEvolution,
		"qxcmd:symphony:sev.command-surface":       featureSEVSCSEV,
		"qxcmd:symphony:sev.novelty-check":         featureSEVNoveltyWatch,
		"qxcmd:symphony:sev.watch-check":           featureSEVNoveltyWatch,
		"qxcmd:symphony:sev.trigger-coalesce":      featureSEVNoveltyWatch,
		"qxcmd:symphony:sev.session-bind":          featureSEVLifecycleBinding,
		"qxcmd:symphony:sev.graph":                 featureSEVEvolution,
...
func TestReviewedBackendFeatureBindingsReachExpectedRegistry(t *testing.T) {
	if len(reviewedBackendFeatureBindings) != 87 {
		t.Fatalf("reviewed backend command count = %d, want 87", len(reviewedBackendFeatureBindings))
	}
	...
	if secondaryBindingCount != 102 {
		t.Fatalf("reviewed backend binding count = %d, want 102", secondaryBindingCount)
	}
