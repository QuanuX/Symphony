...
func printUsage() {
	...
	fmt.Println("  scv inspect|provider-onboard|source-plan|source-check|capture-import|capture-compare|interpret|graph|query|diff|explain|evaluate --prefix PATH [--domain DOMAIN] --input FILE [--json] Invoke exact SCV domain engine")
	fmt.Println("  scv acquire --prefix PATH --input FILE [--domain DOMAIN] [--json] Collect bounded public HTTPS text with exact source and redirect evidence")
	fmt.Println("  scv source propose|apply|status|recover --prefix PATH --tops-id UUID --source-id ID [...] Administer protected noncanonical source revisions")
	fmt.Println("  scv projection select|status|recover --prefix PATH --tops-id UUID --graph-id ID [...] Administer one coherent noncanonical graph selection")
	fmt.Println("  --help                            Print concise usage")
	fmt.Println("  --version                         Print version")
	fmt.Println("  commands manifest --json          Emit digest-bound observed command registry")
	fmt.Println("  commands expected --json          Emit client-independent expected command registry")
	fmt.Println("  commands verify --input FILE --json Verify a packaged expected registry against this command tree")
	...
	fmt.Println("  validate root-summary --prefix PATH [--repo PATH] [--json] Project exact machine-checked root summary evidence")
	...
	fmt.Println("  ssiag provider binding apply <name> --tops-id UUID --plan-digest DIGEST --expected-state-digest STATE [--json] Audit and apply an exact provider binding plan")
	fmt.Println("  ssiag provider binding apply-status <name> --tops-id UUID --operation-id UUID [--json] Read one provider binding attempt")
	fmt.Println("  ssiag provider binding recover <name> --tops-id UUID --expected-state-digest STATE --reason TEXT [--json] Recover the unique provider binding attempt")
	...
	fmt.Println("  ssiag enrollment|supervisor status|plan|apply|apply-status|recover --prefix PATH --tops-id UUID [...] Administer exact receipt-v2 foundational lifecycle surfaces")
	...
	fmt.Println("  stav accordare status|reconcile|supervisor-install|supervisor-uninstall --tops-id UUID [--scope user|system] [--json] Administer durable Accordare audit state and native liveness")
