...
The Architect-ratified STAV v1 JSON Schema/conformance fixtures, fifty-three common SKV process/descriptor/receipt/binding/proposal/provider-evidence/reconciliation/session/SSFV-maintenance/lifecycle/ownership/temporal/Maestro/validation schemas (fifty v1 and three v2), twelve SSIAG authorization/grant-planning/policy-administration schemas, four SKVI operation/result schemas, five SCLV v3 operation/result schemas, six SACV v1 operation/result schemas, eight SODV operational schemas, and eighteen SSFV v1/v2 schemas are canonical protocol truth, not generated projections. The artifact checker may allow only their 134 exact paths and must emit `artifact.canonical_json_authorized` evidence for every encountered file with its exact vector or umbrella authority. Prefix or extension-wide JSON allowlisting is prohibited.
