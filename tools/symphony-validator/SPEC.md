...
## Exit-code Model
`0` to `26`, mapping to passes, errors, blockers, malformed repositories, or internal failures. The exact mapping is:
...
- `24`: feature-administration profile, SSFV registry binding, expected command registry, module-admission census, or cross-reference failure;
...
The Architect-ratified STAV conformance artifacts, common SKV v1/v2 protocols and invariant registry, SSIAG, SKVI, SCLV, SACV, SODV, SSFV, SAV and SEV schemas, plus the six SCV source-knowledge schemas and one explicitly synthetic replay fixture, are canonical contract/evidence artifacts rather than uncontrolled installation projections. The exact 230 admitted paths are enumerated in `src/artifacts.cpp`; every encountered admitted JSON path emits `artifact.canonical_json_authorized` with its owner. The SCV synthetic fixture asserts no vendor capability. Prefix or extension-wide JSON allowlisting is prohibited; private source/graph stores are not admitted into canonical knowledge by this rule.
...
The checker also enumerates direct children of `modules/` without following symlinks and applies a bounded 1,024-module ceiling. `CMakeLists.txt`, `go.mod`, `Package.swift`, or a direct `src`, `cmd`, or `Sources` directory is an implementation marker. Every implemented scope must expose an exact no-follow `<module>/FEATURES.md`, route that exact scope/file pair from the SSFV registry, and have at least one routed feature represented in the profile. A direct module containing none of those markers remains documentation-only and is not promoted from a Contract Quad proposal seed by the census. Discovery, unsafe-marker, missing-file, missing-route, and missing-profile evidence use `feature_administration.module_admission`; failure retains exit `24`.

### Invariant Ownership Assurance Boundary

When `knowledge/INVARIANT-OWNERSHIP.json` is absent, the validator emits explicit absence evidence for repositories predating the contract. When present, it must be a no-follow regular file bounded to 4 MiB and conform to the exactly selected v1 or v2 fixed values and closed field sets. V1 retains its original adapter restrictions. V2 admits explicit generic format-2 engine adapters while preserving legacy format-1 identity pairs. `registry_digest` is tagged SHA-256 over recursively key-sorted compact JSON with that field omitted. Invariant and adapter identities use their exact schema grammar; invariants, adapters, operations, paths, cases, and references are sorted and unique where the canonical contract requires it.

Every owner contract, producer implementation, and regression path must resolve through a bounded no-follow read. Every named Go, C++, Swift or supported shell regression must have a mechanically discoverable source definition. Producer and consumer roles may not be satisfied by the same path/case relation. Allowed adapters must resolve, each operation has one adapter owner, and every registered adapter must be referenced. An IPC invariant requires real-process evidence for each allowed adapter, and that source must visibly execute a process with standard input and captured output. Generic v2 engine adapters additionally bind exact component, receipt entrypoint, owner contract and operation namespace; Native C++ process evidence binds receipt verification, bounded process execution with stdin and captured output, and exact response assertions through the declared test support. These checks prove bounded identity, traceability, and declared boundary mechanics; named source presence does not prove that a test executed or passed. Applicable suites must be run by build/test orchestration. Failure returns exit `26`.

The checker never executes a referenced test, loads an adapter, invokes qxctl, mutates the registry, or assumes catalog completeness. qxctl `knowledge invariant status|list|show` are bounded consumer projections with `semantic_validity: not_asserted`; `qxctl knowledge invariant check` mediates the complete validator result rather than elevating those projections into semantic proof.
...
The validator may report failures and identify expected/observed conditions. It must not rewrite files or choose remedies. Direct `apply` invocation returns stable invalid-usage status `1` with `error: apply is unavailable; symphony-validator is read-only`. Root-summary and complete validation preserve repository bytes on success and failure.
...
The checker scans `README.md` at the repository root when present, plus Markdown files beneath `knowledge/`, `modules/`, `libraries/`, `tools/qxctl/`, and `tools/symphony-validator/`. A directory named `build` is pruned at any depth. `tools/symphony-validator/tests/` is excluded so adversarial fixtures do not become repository findings. Markdown fixtures beneath `knowledge/stav/fixtures/` remain in scope; canonical non-Markdown STAV fixtures are outside this check.
...
The implemented validator parser reads the three fixed canonical-surface bootstrap paths and only the owner manifests explicitly delegated by `knowledge/MANIFEST.md` when computing required closure. Independent bounded checks may also read applicable canonical Markdown surfaces including:
...
## First-party repository source policy

The validator rejects Python source, interpreter entry points, dependency manifests and executable build/test dependencies in first-party repository surfaces. It reports misplaced repository-root `INTENT.md`, `FEATURES.md` and `research/ssiag-phase10b-e`; platform governance belongs under `knowledge/platform/`, and archival research remains external evidence. This rule checks the supplied source repository, skips Git metadata and build outputs, and neither removes files nor controls user-authored programs. Historical ledger prose is not an executable dependency. The fixed bound is 100,000 inspected entries, including directories, and 8 MiB per build file.
