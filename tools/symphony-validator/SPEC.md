...
The owner-ratified STAV v1 JSON Schema/conformance fixtures and the four common SKV engine-process/descriptor/receipt schemas are canonical protocol truth, not generated projections. The artifact checker may allow only their 32 exact paths and must emit `artifact.canonical_json_authorized` evidence for every encountered file with `knowledge/stav/SPEC.md` or `knowledge/SPEC.md` authority as applicable. Prefix or extension-wide JSON allowlisting is prohibited; any new canonical JSON artifact requires an explicit contract and validator update.
