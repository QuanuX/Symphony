...
When `modules/maestro/` exists, the validator requires its Contract Quad, install contract, C++26 build surface, exact qxctl command/client anchors, SKVI coverage, and six common Maestro schema allowlist entries. It validates repository contract shape only; it never opens a TOPS presence registry, mutates docking state, or treats presence as canonical knowledge.
