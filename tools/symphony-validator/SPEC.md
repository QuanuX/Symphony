...
Every owner contract, producer implementation, and regression path must resolve through a bounded no-follow read. Every named Go, C++, Swift, supported shell or Python regression must have a mechanically discoverable source definition. Producer and consumer roles may not be satisfied by the same path/case relation. Allowed adapters must resolve, each operation has one adapter owner, and every registered adapter must be referenced. An IPC invariant requires real-process evidence for each allowed adapter, and that source must visibly execute a process with standard input and captured output. Generic v2 engine adapters additionally bind exact component, receipt entrypoint, owner contract and operation namespace; Python process evidence must include subprocess stdin, captured output, exit handling and receipt evidence. These checks prove bounded identity, traceability, and declared boundary mechanics; named source presence does not prove that a test executed or passed. Applicable suites must be run by build/test orchestration. Failure returns exit `26`.
