...
if [ "$(printf '%s\n' "$OUT_REPO" | grep -c "artifact.canonical_json_authorized")" -ne 80 ]; then
    echo "error: current repo should authorize exactly the 28 STAV, 11 common SKV, 4 SKVI, 5 SCLV, 6 SACV, 8 SODV, and 18 SSFV JSON artifacts"
    exit 1
fi
